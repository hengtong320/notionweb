(() => {
  ...
  const BLESSING_CARDS=[
    ...
    {path:'assets/blessings-v42/07-peony-courtyard.webp',name:'牡丹晨露',title:'花开见喜',english:'BLOOMING JOY',lines:['花开富贵','喜乐常伴'],accent:'#ffb3c7',titleColor:'#fffaf5',scheme:'realistic-v42'},
  ...
  const REALISTIC_BLESSING_COUNT=18;
  ...
  const dom = {
    ...
    settingsModal: $('settingsModal'), galleryModal: $('galleryModal'), blessingModal: $('blessingModal'), levelIntro: $('levelIntro'),
    ...
    tutorialHand: $('tutorialHand'), timeText: $('timeText'), moveText: $('moveText'), progressBar: $('progressBar'), workBadge: $('workBadge'), workBadgeCount: $('workBadgeCount'),
  ...
  const CLASSIC_TILE_ASPECT=0.69;
  const BLESSING_TILE_ASPECT=0.75;
  let TILE_ASPECT=CLASSIC_TILE_ASPECT;
  ...
  function loadSave() {
    ...
    try {
      ...
      return {
        ...
        blessingLevel: Math.max(1,Number(parsed.blessingLevel)||1)
      };
    } catch (error) {
  ...
  function showModal(modal) {
    [dom.settingsModal, dom.galleryModal, dom.blessingModal].forEach((el) => el.classList.toggle('is-visible', el === modal));
  }
  function hideModals() {
    [dom.settingsModal, dom.galleryModal, dom.blessingModal].forEach((el) => el.classList.remove('is-visible'));
  }
  ...
  function generateLevel(level) {
    const seed=((game.mode==='blessing'?0xB1E55100:0)+(level*2654435761+9109))>>>0;
  ...
  async function commitMove(sourceGroup, dr, dc, preparedBoard = null, fromDrag = false) {
    ...
    if (!save.tutorialSeen&&game.level===1&&game.mode==='classic') {
  ...
  async function animateAndClear(groups,tier=1) {
    ...
    for(const group of groups){
      ...
      overlay.style.backgroundImage=`url("${PICTURE_PATHS[group.imageIndex]}")`;overlay.style.animationDuration=`${Math.max(330,650-tier*55)}ms`;
    ...
    game.clearedCount+=groups.length;game.movesSinceClear=0;renderBoard();updateHud();
  ...
  async function startLevel(level) {
    ...
    const stage=document.getElementById('gameStage');stage?.classList.toggle('is-hard',hard);stage?.classList.toggle('is-blessing',game.mode==='blessing');stage?.classList.toggle('is-realistic-blessing',game.mode==='blessing');
    const title=dom.levelNumber.parentElement;title?.classList.toggle('is-hard',hard);
    const titleLabel=title?.querySelector('span');if(titleLabel)titleLabel.textContent=game.mode==='blessing'?'祝福':(hard?'困难':'关卡');
  ...
  function renderGallery() {
    ...
    PICTURE_PATHS.slice(0,STANDARD_PICTURE_COUNT).forEach((src,index)=>{
  ...
  function unlockedBlessingIndices(){return [...new Set(save.blessings.filter(isBlessingIndex))];}
  ...
  function roundedRectPath(ctx,x,y,w,h,r){return posterRoundRect(ctx,x,y,w,h,r);}
  ...
  function bindEvents() {
    dom.playBtn.addEventListener('click',()=>{audio.tap();game.mode='classic';startLevel(save.level);});
    dom.blessingBtn.addEventListener('click',()=>{audio.tap();game.mode='blessing';startLevel(save.blessingLevel||1);});
    dom.homeWorksBtn.addEventListener('click',()=>{audio.tap();openBlessingWorks();});
    ...
    dom.workBadge.addEventListener('click',()=>{audio.tap();openBlessingWorks();});
    ...
    dom.blessingPrevBtn.addEventListener('click',()=>{audio.tap();shiftBlessing(-1);});dom.blessingNextBtn.addEventListener('click',()=>{audio.tap();shiftBlessing(1);});
    ...
    [dom.settingsModal,dom.galleryModal,dom.blessingModal].forEach((layer)=>layer.addEventListener('pointerdown',(event)=>{if(event.target===layer){if(layer===dom.settingsModal)closeSettings();else hideModals();}}));
  ...
  async function boot() {
    ...
    await preloadImages([((save.level-1)*5)%STANDARD_PICTURE_COUNT]);
    await delay(850);dom.splash.classList.remove('is-visible');
    const params=new URLSearchParams(location.search),requested=Number(params.get('level'));
