...
export function initLearningEnhancements(ctx) {
 ...
 ctx.ACUPOINTS=ctx.ACUPOINTS.map(p=>({...p,name:evidenceData.points[p.code]?.name||p.name,aliases:evidenceData.points[p.code]?.aliases||[],pinyin:correctedPinyin(evidenceData.points[p.code]?.name||p.name,p.pinyin)}));
 ...
 function focusPoint(pos){
  ...
  if(precisionMode==='strict'){
   ...
   if(box.isEmpty()){studyContext=null;const map={chest:'thorax',abdomen:'lumbosacral',back:'spine',neck:'cervical',leg:'leg',thigh:'whole',forearm:'upper',pelvis:'pelvis',head:'head',hand:'hand',foot:'foot',upper:'upper'};const region=map[p?.region];if(region)ctx.setRegion(region);toast('该坐标未校准：仅查看相关解剖区域，不显示精确点');updateOverlayVisibility();return true;}
 ...
 window.dispatchEvent(new CustomEvent('atlas:v9-point-click',{detail:{code:p.code}}));
