...
export function initLearningEnhancements(ctx) {
 ...
 let studyContext=null,curveStyle='smooth';
 ...
 for(const m of ctx.MERIDIANS){const sideList=PAIR.has(m.id)?['right','left']:m.id==='EX'?['right','left','midline']:['midline'];
  for(const side of sideList){const members=ctx.ACUPOINTS.filter(p=>p.meridian===m.id);const data=[];
   ...
   const guides=createGuidePaths(THREE,bones,m.id,side,data).map(path=>{
    const sampled=smoothGuidePath(THREE,path.points,10),smooth=sampled.points;
    ...
    return {guide,border,points:smooth,note:path.note,curveDiagnostics:sampled.diagnostics};
   });
 ...
 function updateOverlayVisibility(){
  ...
  if(selectedMarker){selectedMarker.visible=enabled&&!suspended&&pointsOn&&!!(selectedPoint?.position||selectedPoint?.navigationArea);selectedMarker.material.depthTest=!xray;}
  const active=routeRecords.filter(routeMatches),gn=active.reduce((n,r)=>n+r.guides.length,0);
  if($('guideCoverage'))$('guideCoverage').textContent=!guideOn?'线路已关闭；点位和穴名仍可查看。勾选走向导览恢复线路。':gn?`${curveStyle==='smooth'?'平滑线':'虚线'}：区域导览，非真实神经或取穴线；圆点：原穴名参照。${gn} 段；仅平滑显示，未改变穴位坐标。`:'所选条目暂无走向线；点位或名称目录仍可查看。';
 ...
 if(focus&&(p.position||p.navigationArea)){focusReference(p);}if(focus&&!p.position&&!p.navigationArea){document.body.classList.add('detail-open');toast('尚无可导航位置，保留资料说明');}mountLayerContext();if(autoSpeak())speak(p.name);return true;
 }
