...
export function initLearningEnhancements(ctx) {
 ...
 function updateOverlayVisibility(){
  ...
  if($('guideCoverage'))$('guideCoverage').textContent=referenceWindow()?'局部观察：'+studyContext.name+'；只显示当前范围内的点线。点击“看全线”恢复整条，或在图层设置关闭局部范围。':!guideOn?'线路已关闭；点位和穴名仍可查看。勾选走向导览恢复线路。':gn?`${curveStyle==='smooth'?'平滑线':'虚线'}：区域导览，非真实神经或取穴线；圆点：原穴名参照。${gn} 段；仅平滑显示，未改变穴位坐标。`:'所选条目暂无走向线；点位或名称目录仍可查看。';
