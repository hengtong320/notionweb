import {correctRegionalReferences,createRegionalGuides} from './route-corridors-v13.js';
...
export function initLearningEnhancements(ctx) {
 ...
 ctx.ACUPOINTS=correctRegionalReferences(ctx.ACUPOINTS.map(canonicalPoint));
 ...
 let surfaceProjector=null,surfaceAttached=false,surfaceBusy=false;let studyContext=null,curveStyle='tube',precisionMode='strict';
 ...
 function toggleTCM(on){enabled=!!on;window.dispatchEvent(new CustomEvent('atlas:tcm-visibility'));routeRoot.visible=enabled;updateOverlayVisibility();if(!enabled){card.hidden=true;cardOpen=false;hover.hidden=true;}updateStatus();}
 function setMeridians(ids){ctx.invalidate();labelPage=0;selectedMeridians=new Set(ids.filter(id=>meridianMap[id]));activeMeridian=selectedMeridians.size===1?[...selectedMeridians][0]:'MULTI';$('meridianQuick').value=activeMeridian;panel.querySelectorAll('[data-meridian]').forEach(b=>{b.classList.toggle('active',selectedMeridians.has(b.dataset.meridian));b.setAttribute('aria-pressed',String(selectedMeridians.has(b.dataset.meridian)));});if(selectedPoint&&!selectedMeridians.has(selectedPoint.meridian)){selectedPoint=null;card.hidden=true;cardOpen=false;clearSelectedMarker();}updateOverlayVisibility();renderResults();window.dispatchEvent(new CustomEvent('atlas:selection',{detail:{kind:'meridian',id:[...selectedMeridians].join(','),name:[...selectedMeridians].map(id=>meridianMap[id].name).join('＋'),side:tcmSide}}));}
 ...
 function setTCMSide(side){ctx.invalidate();
  ...
  if(prior&&prior.side!=='midline'&&side!=='both'){
   ...
   if(target){selectPoint(target,false);return;}
  }
 ...
 for(const m of ctx.MERIDIANS){const sideList=PAIR.has(m.id)?['right','left']:m.id==='EX'?['right','left','midline']:['midline'];
  for(const side of sideList){const members=ctx.ACUPOINTS.filter(p=>p.meridian===m.id);const data=[];
   ...
   const guides=createRegionalGuides(THREE,bones,m.id,side,data,createGuidePaths).map(path=>{
 ...
 function updateOverlayVisibility(){ctx.invalidate();if(surfaceAttached)xray=false;
 ...
 function fitMeridian(requestedDirection=null){if(state.bodySex==='female')return false;
 ...
 function selectPoint(p,focus=false){if(!p?.name||!p?.code)return false;ctx.invalidate();p=canonicalPoint(p);if(state.bodySex==='female'){window.__ATLAS_FEMALE__?.clearSelection();p={...p,position:null,navigationArea:null,landmarks:[]};toggleTCM(true);}document.body.classList.add('reference-detail');
 ...
 card.innerHTML=pointCardV9(p,m,BY_ID);if(state.bodySex==='female'){$('focusPointBtn').disabled=true;$('focusPointBtn').textContent='此模型暂无点位';const hint=document.createElement('p');hint.className='shared-coverage';hint.textContent='介绍与名称共用；女性三维点位未标注，当前视角不变。';card.querySelector('.tcm-card-actions').after(hint);}
 ...
 function setStudyContext(p){if(state.bodySex==='female'){studyContext=null;return;}
 ...
 function focusReference(p){if(state.bodySex==='female')return false;
 ...
 function mountLayerContext(){
  ...
  card.append(box);
  if(state.bodySex==='female'){box.querySelector('h4').textContent='在当前视角看结构';box.querySelector('p').textContent='切换图层不改变视角；女性点位尚未标注。';}for(const b of box.querySelectorAll('[data-study-profile]'))b.onclick=async()=>{await window.__ATLAS_SHARED__?.choose(b.dataset.studyProfile,true,{keepSection:true});refreshLayerContext();};
  refreshLayerContext();
 }
 ...
 function pointUnoccluded(p){
  if(surfaceAttached&&surfaceProjector)return surfaceProjector.isVisible(p.position,camera.position);if(xray)return true;
 ...
 function hitRoute(e){
  ...
  for(const r of routeRecords.filter(routeMatches))for(const g of r.guides)for(let i=1;i<g.points.length;i++){
   ...
   const d=Math.hypot(e.clientX-a.x-t*dx,e.clientY-a.y-t*dy);if(d<bestDistance&&(!surfaceAttached||surfaceProjector.isVisible(g.points[i-1].clone().lerp(g.points[i],t),camera.position))){bestDistance=d;best=r;}
  }return best;
 }
 function handlePointerClick(e){if(state.mode!=='orbit'||e.button!==0)return false;const p=hitPoint(e);if(p)return selectPoint(p,autoFocus);const r=hitRoute(e);if(!r)return false;setMeridian(r.meridian);setTCMSide(r.side==='midline'?'both':r.side);toast(meridianMap[r.meridian].name+' · 已选择，视角保持');window.dispatchEvent(new CustomEvent('atlas:v9-channel-click',{detail:{ids:[r.meridian]}}));return true;}
 ...
 function updatePointLabels(){
  ...
  let candidates=all.filter(p=>!surfaceAttached||surfaceProjector.isVisible(p.position,camera.position)).map(p=>({p,v:project(p)})).filter(x=>inView(x.v));
  ...
  wires.innerHTML=paths;labelStats={total:all.length,inView:count,shown,page:labelMode==='complete'?labelPage+1:1,pages:labelMode==='complete'?pages:1,capacity};
  $('labelPageInfo').textContent=labelMode==='complete'?`${shown}/${count} 视野内 · ${labelPage+1}/${pages}页`:`附近${shown}个 · 完整目录${all.length}个`;
  $('labelPrevious').disabled=labelMode!=='complete'||labelPage===0;$('labelNext').disabled=labelMode!=='complete'||labelPage>=pages-1;
 }
 ...
 async function setSurfaceAttachment(on){
  if(surfaceBusy)return false;if(on&&!surfaceProjector)return false;if(surfaceAttached===!!on)return true;
  surfaceBusy=true;const saved=selectedPoint&&{code:selectedPoint.code,side:selectedPoint.side};
  try{
   for(const r of routeRecords){
    for(const p of r.data){if(!p.sourcePosition)p.sourcePosition=p.position.clone();p.position.copy(on?surfaceProjector.project(p.sourcePosition,2.2,{meridian:r.meridian,side:r.side,region:p.region,view:p.view}):p.sourcePosition);}
    for(const g of r.guides){
     if(!g.sourcePoints){g.sourcePoints=g.points.map(v=>v.clone());g.sourceGeometry=g.tube.geometry;}
     if(on&&!g.projected){g.projected=surfaceProjector.curve(g.sourcePoints,{meridian:r.meridian,side:r.side});}
     g.tube.geometry=on?g.projected.geometry:g.sourceGeometry;g.points=on?g.projected.points:g.sourcePoints;
     const geometry=new LineGeometry().setPositions(g.points.flatMap(v=>v.toArray()));g.guide.geometry.dispose();g.guide.geometry=geometry;g.border.geometry=geometry;g.guide.computeLineDistances();g.border.computeLineDistances();
    }
    await new Promise(resolve=>setTimeout(resolve,0));ctx.invalidate();
   }
   if(on){
   ...
   surfaceAttached=!!on;surfaceBadge.hidden=!on;lastReferenceWindowKey='';lastLabelRebuild='';lastLabelsAt=0;
   if(on){xray=false;$('tcmXray')&&($('tcmXray').checked=false);}
   if(saved){const p=pointIndex.get(saved.code+'|'+saved.side)||pointIndex.get(saved.code+'|midline');if(p)selectPoint(p,false);}
   syncReferenceWindow();updateOverlayVisibility();ctx.invalidate();return true;
  }finally{surfaceBusy=false;}
 }

 window.__ATLAS_LEARNING__={cancelPendingFocus:()=>{focusSerial++;navigationFocus=null;},setSurfaceProjector:p=>{surfaceProjector=p;},setSurfaceAttachment,getSurfaceAudit:()=>routeRecords.filter(routeMatches).flatMap(r=>r.guides.flatMap(g=>g.points.filter((v,i)=>i%Math.max(1,Math.floor(g.points.length/30))===0).map(position=>({screen:project({position}),visible:surfaceProjector?.isVisible(position,camera.position)})))),getSurfaceState:()=>({attached:surfaceAttached,originalRecordsPreserved:true,navigationRevision:13,clinicalCalibration:false}),clearStudyContext,focusSelectedPoint:()=>selectedPoint?focusReference(selectedPoint):false,flushLabels:()=>{lastLabelsAt=0;updatePointLabels();},setLabelMode:mode=>{labelMode=mode==='complete'?'complete':'smart';$('labelModeToggle').textContent=labelMode==='complete'?'完整穴名':'附近穴名';lastLabelRebuild='';ctx.invalidate();},clearPointSelection:()=>{selectedPoint=null;cardOpen=false;card.hidden=true;clearSelectedMarker();updateStatus();},setPrecisionMode:mode=>{precisionMode='illustrative';if(selectedPoint)setStudyContext(selectedPoint);updateOverlayVisibility();lastLabelsAt=0;lastLabelRebuild='';window.dispatchEvent(new CustomEvent('atlas:precision-changed'));ctx.invalidate();},getReferenceWindowState:()=>{const b=referenceWindow();return {active:!!b,bounds:b&&[b.min.toArray(),b.max.toArray()],visible:getVisiblePoints().map(p=>({code:p.code,side:p.side,position:p.position.toArray()})),drawnPoints:routeRecords.filter(routeMatches).reduce((n,r)=>n+r.cloud.geometry.attributes.position.count,0),clippedRouteMaterials:routeRecords.filter(routeMatches).flatMap(r=>r.guides).filter(g=>g.guide.material.clippingPlanes?.length===6).length};},getStudyContext:()=>studyContext&&{...studyContext,center:[...studyContext.center]},getCurveDiagnostics:()=>routeRecords.flatMap(r=>r.guides.map(g=>({meridian:r.meridian,side:r.side,note:g.note,...g.curveDiagnostics}))),hasNavigationFocus:()=>!!navigationFocus&&enabled,restoreNavigationFocus:()=>{if(navigationFocus){const f=navigationFocus;ctx.focusBounds(f.box,{direction:f.direction,up:f.up});}},setLabelPage:value=>{labelPage=Math.max(0,Number(value)||0);lastLabelsAt=0;},getNavigationCatalog:()=>routeRecords.flatMap(r=>r.data.map(p=>({code:p.code,name:p.name,side:p.side,position:p.position.toArray(),sourcePosition:(p.sourcePosition||p.position).toArray(),originalPosition:p.originalPosition||null,navigationRevision:p.navigationRevision||null,displayProjection:surfaceAttached,region:p.region,view:p.view}))),getRouteGeometry:()=>routeRecords.map(r=>({meridian:r.meridian,side:r.side,branches:r.guides.map(g=>g.points.map(v=>v.toArray()))})),getRouteScreen:(id,side='right')=>{const r=routeRecords.find(r=>r.meridian===id&&(r.side===side||r.side==='midline'));return r?.guides.flatMap(g=>g.points.map(position=>project({position})))||[];},setMeridians,suspend:value=>{value=!!value||ctx.state.bodySex==='female';if(suspended!==value){suspended=value;updateOverlayVisibility();}},getReferences:()=>ctx.ACUPOINTS.map(p=>({code:p.code,name:p.name,pinyin:p.pinyin,mapped:p.mapped,region:p.region,hasPosition:!!p.position})),handlePointerClick,hitPoint,toggleTCM,setPanel,setMeridian,setTCMSide,fitMeridian,runPreset,updateFrame,selectPoint:(code,side='right',focus=false)=>selectPoint(pointIndex.get(code+'|'+side)||pointIndex.get(code+'|midline')||{...ctx.ACUPOINTS.find(p=>p.code===code),side,position:null},focus),getPointScreen:(code,side='right')=>{const p=pointIndex.get(code+'|'+side)||pointIndex.get(code+'|midline');return p?.position?project(p):p?.navigationArea?{...project({position:areaCenter(p)}),areaOnly:true}:null;},speakBone:id=>speak(BY_ID[id]?.name),getState:()=>({surfaceAttached,labelMode,precisionMode,coordinateVerified:0,curveStyle,headAreaVisible:!!selectedMarker?.userData.studyArea&&selectedMarker.visible,studyContext,labelStats,autoFocus,navigationFocus:navigationFocus&&{title:navigationFocus.title,serial:navigationFocus.serial,center:navigationFocus.box.getCenter(new THREE.Vector3()).toArray()},guideOn,routeDiagnostics:routeRecords.filter(routeMatches).map(r=>({meridian:r.meridian,side:r.side,markerRenderOrder:r.cloud.renderOrder,guideRenderOrder:Math.max(0,...r.guides.map(g=>g.guide.renderOrder)),referencePoints:r.data.length,connected:r.hasSegments,guideCount:r.guides.length,guideVertices:r.guides.reduce((n,g)=>n+g.points.length,0),visible:precisionMode==='illustrative'&&enabled&&!suspended&&(r.line.visible||r.guides.some(g=>g.guide.visible||g.tube.visible))})),version:'13.0.0',termSource:'catalog-v10',selectedMeridians:[...selectedMeridians],suspended,legacyEqualSpacingRemoved:true,clinicalCalibration:false,enabled,panelOpen,linesOn,pointsOn,namesOn,xray,tcmSide,activeMeridian,selectedPoint:selectedPoint&&{code:selectedPoint.code,name:selectedPoint.name,side:selectedPoint.side,areaOnly:!!selectedPoint.navigationArea},cardOpen,statusVisible:!status.hidden,meridians:14,supplementalNames:12,standardAcupoints:361,annotatedNames:ctx.ACUPOINTS.filter(p=>p.mapped).length,markerInstances:routeRecords.reduce((n,r)=>n+r.data.length,0),visiblePoints:getVisiblePoints().length,routes:routeRecords.length,autoSpeak:autoSpeak(),lineWidthPixels:activeMeridian==='ALL'?2.2:3.5,pointSizePixels:activeMeridian==='ALL'?7:11})};
