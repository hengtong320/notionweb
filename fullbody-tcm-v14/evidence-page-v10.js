...
const hash=new URLSearchParams(location.hash.slice(1));let tab=hash.get('kind')==='meridian'?'meridian':'point',selected=hash.get('id')||'PC6',side=['right','left','both','midline'].includes(hash.get('side'))?hash.get('side'):'right',mode=hash.get('mode')==='illustrative'?'illustrative':'strict',layer=['bones','muscles','nerves','compare','chest','heart','abdomen','vascular','ear-right','ear-left','surface'].includes(hash.get('layer'))?hash.get('layer'):'bones';
const refSex=hash.get('sex'),refPreset=hash.get('preset'),refDisplay=hash.get('display');function contextParams(values){const h=new URLSearchParams(values);if(refSex==='female'){h.set('sex','female');h.set('preset',refPreset||'surface');h.set('display',refDisplay||'solid');}return h;}function targetLink(id=selected){const u=new URL('./',location.href);u.hash=contextParams({kind:tab,id,side,mode,layer}).toString();return u.href;}
function record(){const h=contextParams({kind:tab,id:selected,side,mode,layer});try{history.replaceState(null,'','#'+h.toString());}catch{}if($('return3d'))$('return3d').href=targetLink();}
function show(id){
...
function render(){
 ...
 for(const p of list){const id=p.code||p.id,b=document.createElement('button');b.type='button';b.dataset.id=id;b.innerHTML=`<b>${esc(p.name)}<small>${esc(p.pinyin)}</small></b><span>${esc(id)}</span>`;b.classList.toggle('active',id===selected);b.onclick=()=>{show(id);if(innerWidth<=760)document.getElementById('focusHeading')?.scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});};$('results').append(b);}
...
window.addEventListener('hashchange',()=>{const h=new URLSearchParams(location.hash.slice(1));tab=h.get('kind')==='meridian'?'meridian':'point';side=['left','right','midline','both'].includes(h.get('side'))?h.get('side'):'right';mode=h.get('mode')==='illustrative'?'illustrative':'strict';layer=['bones','muscles','nerves','compare','chest','heart','abdomen','vascular','ear-right','ear-left','surface'].includes(h.get('layer'))?h.get('layer'):'bones';render();show(h.get('id')||'PC6');});
window.__EVIDENCE_PAGE__={DATA,getState:()=>({tab,selected,side,mode,layer}),show};render();show(selected);
