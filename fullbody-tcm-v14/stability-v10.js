...
const profiles=new Set(['bones','muscles','nerves','compare','chest','heart','abdomen','vascular','ear-right','ear-left','surface']);
...
export function initStability(ctx){
 ...
 function linkFor(target=current,base='./'){
  ...
  if(window.__ATLAS_FEMALE__?.active){const f=window.__ATLAS_FEMALE__.getState();hash.set('sex','female');hash.set('preset',f.preset);hash.set('display',f.mode);}u.hash=hash.toString();return u.href;
 }
 function update(){
  if(window.__ATLAS_FEMALE__?.active){document.body.dataset.currentKind=current.kind;$('currentStudyName').textContent=current.name||current.id;$('currentStudyMode').textContent=['point','meridian'].includes(current.kind)?'经穴介绍':'解剖结构';$('speakCurrent').disabled=current.kind==='region';$('evidenceCurrent').href=linkFor(current,'evidence.html');$('evidenceCurrent').textContent=['point','meridian'].includes(current.kind)?'查阅同一条目 ↗':'来源与说明';$('backCurrent').disabled=true;invalidate();return;}
  ...
  $('currentStudyMode').textContent=(current.kind==='point'||current.kind==='meridian')?(p==='strict'?'资料查阅':'经穴示意'):'解剖结构';
 ...
 async function navigate(target){
  ...
  try{
   if(target.kind==='point'&&!learning.getReferences().some(p=>p.code===target.id))throw Error('没有这个穴名，请从目录重新选择');
   if(target.kind==='bone'&&!ctx.boneInfo(target.id))throw Error('没有这个骨名，请从目录重新选择');
   target={...target};if(['point','meridian'].includes(target.kind)&&['chest','heart','abdomen','vascular','ear-right','ear-left','surface'].includes(target.layer)){await tissues.setProfile(target.layer);if(serial!==restoreSerial)return;target.layer=null;}
   ...
   if(target.kind==='point')learning.selectPoint(target.id,target.side||'right',true);
    ...
    if(!known){const system=target.id.split('-')[0];if(['heart','visceral','vessels','ear','surface'].includes(system))await tissues.enable(system,true);else{await tissues.enable('muscular',true);if(serial!==restoreSerial)return;await tissues.enable('nervous',true);}}
    if(serial!==restoreSerial)return;
   ...
   if(serial===restoreSerial&&target.kind!=='tissue'&&target.layer&&profiles.has(target.layer))await tissues.setProfile(target.layer);
 ...
 function focusCurrent(){
  ...
  const profile=tissues.getState().profile;if(['chest','heart','abdomen','vascular','ear-right','ear-left','surface'].includes(profile))return tissues.setAnatomyView(profile);ctx.setRegion(current.id);return true;
 }
 ...
 function parseHash(){const h=new URLSearchParams(location.hash.slice(1));if(h.get('sex')==='female')return null;const kind=h.get('kind'),id=h.get('id');if(!safeKinds.has(kind)||!id||id.length>240)return null;return{kind,id,side:['right','left','both','midline'].includes(h.get('side'))?h.get('side'):'right',mode:h.get('mode')==='illustrative'?'illustrative':'strict',layer:profiles.has(h.get('layer'))?h.get('layer'):'bones'};}
 ...
 async function capture(){
  ...
  const visible=e=>{if(!e)return false;for(let a=e;a instanceof Element;a=a.parentElement){const s=getComputedStyle(a);if(a.hidden||s.display==='none'||s.visibility==='hidden'||s.opacity==='0')return false;}const style=getComputedStyle(e),b=e.getBoundingClientRect();return style.display!=='none'&&style.visibility!=='hidden'&&b.width>0&&b.height>0&&b.right>r.left&&b.left<r.right&&b.bottom>r.top&&b.top<r.bottom;};
  ...
  const mode=(learning.getState().surfaceAttached?'体表显示投影 · ':'')+((current.kind==='point'||current.kind==='meridian'||learning.getState().enabled)?(learning.getState().precisionMode==='strict'?'资料查阅 · 未校准点线隐藏':'三维示意 · 穴位与经络位置未校准'):'解剖教学模型 · 非患者诊断');
  ...
  g.fillStyle='rgba(248,252,245,.96)';g.fillRect(0,r.height-64,r.width,64);g.fillStyle='#345642';g.font='12px "Noto Sans CJK SC",sans-serif';g.fillText(mode+layer,14,r.height-41,r.width-28);g.font='10px sans-serif';g.fillText('V12 · Z-Anatomy / BodyParts3D · CC BY-SA 4.0 · 学习参考',14,r.height-20,r.width-28);g.restore();
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw Error('截图导出失败');
  const filename=`人体研习-${current.name}-${current.id}-V12-${new Date().toISOString().slice(0,10)}.png`.replace(/[\\/:*?"<>|]/g,'-');
  lastCapture={filename,object:clone(current),labels:captured,layerProfile:tissues.getState().profile,mode:mode+layer,bytes:blob.size,width:canvas.width,height:canvas.height};
...
// Whole-body navigation restores the sidebar without changing layer visibility.
