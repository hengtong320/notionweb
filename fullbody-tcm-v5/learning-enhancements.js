...
import {createGuidePaths} from './route-guides-v5.js';
...
export function initLearningEnhancements(ctx) {
 ...
 $('guideBack').onclick=()=>{ctx.setView('back');fitMeridian();};$('guideFront').onclick=()=>{ctx.setView('front');fitMeridian();};
 ...
 function setMeridian(id){id=String(id).toUpperCase().replace(/^DU$/,'GV').replace(/^(RN|REN)$/,'CV');setMeridians(id==='ALL'?ctx.MERIDIANS.map(m=>m.id):[id]);if(id==='GV'||id==='BL'){ctx.setView('back');fitMeridian();}else if(id==='CV'){ctx.setView('front');fitMeridian();}}
 ...
 function updateOverlayVisibility(){
  ...
  if($('guideCoverage'))$('guideCoverage').textContent=!guideOn?'当前仅连接编号连续的参考点；未标注段不连线。勾选走向导览可查看示意虚线。':gn?`虚线：走向导览；圆点：已收录的参考锚点。未标注的穴位只在目录，不补假点。当前 ${gn} 段导览。`:'所选条目暂无走向线；点位或名称目录仍可查看。';
  lastLabelRebuild='';updateStatus();
 }
 function fitMeridian(){
  const active=routeRecords.filter(routeMatches);const pts=active.flatMap(r=>[...r.data.map(p=>p.position),...(guideOn?r.guides.flatMap(g=>g.points):[])]);
  if(!pts.length){toast('此条目仅有名称目录，暂无三维参照；不是加载失败');return;}
  setRegion('body',false);ctx.focusBounds(new THREE.Box3().setFromPoints(pts).expandByScalar(30));if(compact())setPanel(false);
 }
