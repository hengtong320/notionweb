...
export function initLearningEnhancements(ctx) {
 ...
 for(const m of ctx.MERIDIANS){const sideList=PAIR.has(m.id)?['right','left']:m.id==='EX'?['right','left','midline']:['midline'];
  for(const side of sideList){const members=ctx.ACUPOINTS.filter(p=>p.meridian===m.id);const data=[];
   ...
   const guides=createGuidePaths(THREE,bones,m.id,side,data).map(path=>{
    const smooth=[];for(let j=1;j<path.points.length;j++){const a=path.points[j-1],b=path.points[j],steps=Math.max(2,Math.ceil(a.distanceTo(b)/7));for(let k=0;k<steps;k++)smooth.push(a.clone().lerp(b,k/steps));}if(path.points.length)smooth.push(path.points[path.points.length-1].clone());
 ...
 function updateOverlayVisibility(){
  ...
  if($('guideCoverage'))$('guideCoverage').textContent=!guideOn?'线路已关闭；点位和穴名仍可查看。勾选虚线导览恢复线路。':gn?`虚线：体表区域导览；圆点：穴名导航示意（未经临床校准）。当前 ${gn} 段导览。`:'所选条目暂无走向线；点位或名称目录仍可查看。';
 ...
 function renderResults(){const q=normalize($('acupointSearch').value.replace(/影像穴?|迎像穴?/g,'迎香').replace(/穴$/,''));const arr=ctx.ACUPOINTS.filter(p=>(q||selectedMeridians.has(p.meridian))&&(!q||normalize([p.code,p.name,p.pinyin,p.meridianName].join(' ')).includes(q)));$('pointResultCount').textContent=`${arr.length}项 · ${arr.filter(p=>p.mapped).length}可导航`;
 $('acupointResults').innerHTML=arr.map(p=>`<button data-point="${p.code}" class="${selectedPoint?.code===p.code?'active':''}"><b>${p.name}<small>${p.pinyin}</small></b><span>${p.code}<small>${p.mapped?(p.regionLabel||'部位示意'):'资料条目'}</small></span></button>`).join('')||'<p class="empty-search">没有找到</p>';
 ...
 function selectPoint(p,focus=false){if(!p?.name||!p?.code)return false;
 ...
 if(p.position){selectedMarker=new THREE.Points(new THREE.BufferGeometry().setFromPoints([p.position]),makePointMaterial('#d37022',20));selectedMarker.renderOrder=60;routeRoot.add(selectedMarker);selectedMarker.visible=enabled&&!suspended&&pointsOn;}
 ...
 card.innerHTML=`<div class="tcm-card-head"><span>${m.name} · ${p.code}</span><button id="closePointCard">收起</button></div><h3>${p.name}<small>${p.pinyin} · ${p.side==='left'?'人体左侧':p.side==='right'?'人体右侧':'中线／组合'}</small></h3><h4>对应部位 · 导航参照</h4><p class="location-text">${p.locationNote||'此条目已收录名称与归经，位置说明待逐条复核，暂不绘制三维点。'}</p><p class="coordinate-warning">${p.position?'这是按所属部位、骨性标志和分寸关系近似换算的导航示意，未经临床校准。缺少皮肤和部分软组织时只能作区域对照，不能用于取穴。':'缺少相应标志或尚未标注：不生成假坐标，不自动跳到别的骨头。'}</p>${p.sourceScheme?`<small>${p.sourceScheme}</small>`:''}<div class="tcm-card-actions"><button id="focusPointBtn" ${p.position?'':'disabled'}>聚焦此部位</button><button id="speakPointBtn">朗读穴名</button></div><div class="point-step"><button id="pointPrev">上一穴</button><button id="pointNext">下一穴</button></div><details class="bone-reference"><summary>相关骨性参照（不能代替体表取穴）</summary>${(p.landmarks||[]).filter(id=>BY_ID[id]).map(id=>`<button data-near="${id}">${BY_ID[id].name}</button>`).join('')||'待补充'}</details><p><a class="reference-link" href="${p.reference||'https://iris.who.int/handle/10665/353407'}" target="_blank" rel="noopener">查阅定位资料来源 ↗</a></p>`;
 ...
 if(focus&&p.position){focusPoint(p.position);}if(focus&&!p.position){document.body.classList.add('detail-open');toast('该补充穴为资料条目，尚无三维位置；不伪装成聚焦成功');}if(autoSpeak())speak(p.name);return true;
 }
 ...
 function updatePointLabels(){
  ...
  const pool=candidates.filter(x=>x.p!==pin?.p),slots=Math.max(1,capacity-(pin?1:0));
  ...
  let pageItems=labelMode==='complete'?pool.slice(labelPage*slots,(labelPage+1)*slots):pool;
