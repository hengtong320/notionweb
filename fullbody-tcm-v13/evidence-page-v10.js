...
function targetLink(id=selected){const u=new URL('./',location.href);u.hash=new URLSearchParams({kind:tab,id,side,mode,layer}).toString();return u.href;}
function record(){const h=new URLSearchParams({kind:tab,id:selected,side,mode,layer});try{history.replaceState(null,'','#'+h.toString());}catch{}if($('return3d'))$('return3d').href=targetLink();}
function show(id){
