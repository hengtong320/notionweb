...
export function initSpeech(toast){
 ...
 function systemSpeak(text){
  ...
  spokenText=lookupTerm(text)?.spokenText||speechHint(text);const u=new SpeechSynthesisUtterance(spokenText);utterance=u;u.lang='zh-CN';u.voice=selected||null;u.rate=rate;u.pitch=1;
 ...
 function speak(text){if(!text)return false;stop();lastName=text;lastError='';spokenText='';const ticket=serial;
  ...
  const fixed={'长强':'changqiang','长强穴':'changqiang','大椎':'dazhui','龈交':'yinjiao','郄门':'ximen','阴郄':'yinxi'};const fix=fixed[canonical(text)];if(fix){lastEngine='V13 专项读音';spokenText=lookupTerm(text)?.spokenText||speechHint(text);audio.playbackRate=rate;audio.src='./voice-corrected/'+fix+'.mp3';audio.play().catch(e=>{if(ticket!==serial)return;lastError=String(e);systemSpeak(text);});return true;}const guarded=lookupTerm(text)?.readingGuard||requiresReadingGuard(text),key=voiceData.mapping?.[text]||voiceData.mapping?.[canonical(text)];
  if(voice==='neural'&&key&&!guarded){lastEngine='旧预制合成音频（未逐词审听）';spokenText=text;audio.playbackRate=rate;audio.src=window.ATLAS_VOICE_INLINE?.[key]?'data:audio/mpeg;base64,'+window.ATLAS_VOICE_INLINE[key]:(window.ATLAS_RESOURCE_BASE||'./')+'voice/'+key;audio.play().catch(e=>{if(ticket!==serial)return;lastError=String(e);systemSpeak(text);});return true;}
  return systemSpeak(text);
 }
 function mount(container){
  const field=document.createElement('section');field.className='voice-settings';field.innerHTML='<h3>术语朗读</h3><label>声音<select id="voiceChoice" aria-label="朗读音色"></select></label><label>语速<input id="voiceSpeed" type="range" min="0.65" max="1.15" step="0.05"><output id="voiceRate"></output></label><div class="v4-button-row"><button id="voiceSample">试听长强</button><button id="voiceStop">停止</button></div><p class="reading-example">长强 <b>cháng qiáng</b>　大椎 <b>dà zhuī</b>　龈交 <b>yín jiāo</b><br>攒竹 <b>cuán zhú</b>　郄门 <b>xì mén</b></p><small>读音优先使用普通话设备音色和术语提示；多音字不再复用旧版未核音录音。不同设备仍可能读错，带声调拼音用于核对。长强等五个易错词使用专项消歧合成音频；全套音频尚未逐词审听。</small>';container.append(field);
  const refresh=()=>{const select=field.querySelector('#voiceChoice');select.replaceChildren(new Option('读音优先 · 设备普通话','reading'),new Option('旧预制音频 · 易错词回退普通话','neural'));for(const v of available())select.add(new Option(v.name+' · '+v.lang+(v.localService?' · 设备本地':' · 在线服务'),v.voiceURI));select.value=[...select.options].some(o=>o.value===voice)?voice:'reading';};refresh();window.speechSynthesis?.addEventListener('voiceschanged',refresh);
  field.querySelector('#voiceChoice').onchange=e=>{voice=e.target.value;put('atlas-voice-v5',voice);stop();};const range=field.querySelector('#voiceSpeed');range.value=rate;field.querySelector('#voiceRate').textContent=rate.toFixed(2)+'×';range.oninput=()=>{rate=Number(range.value);field.querySelector('#voiceRate').textContent=rate.toFixed(2)+'×';put('atlas-voice-rate',String(rate));};field.querySelector('#voiceSample').onclick=()=>speak('长强');field.querySelector('#voiceStop').onclick=stop;
 }
 window.addEventListener('pagehide',stop);
 return {speak,stop,mount,getState:()=>({voice,rate,lastName,spokenText,lastEngine,lastError,playing:!audio.paused||!!utterance,audioURL:audio.currentSrc||audio.src,currentTime:audio.currentTime,recordings:voiceData.uniqueRecordings||0,synthetic:true,readingGuard:true,termSource:'catalog-v10',remoteVoicePossible:true,allAudioHumanReviewed:false})};
}
