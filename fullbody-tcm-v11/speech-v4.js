...
export function initSpeech(toast){
 ...
 function speak(text){if(!text)return false;stop();lastName=text;lastError='';spokenText='';const ticket=serial;
  ...
  const guarded=lookupTerm(text)?.readingGuard||requiresReadingGuard(text),key=voiceData.mapping?.[text]||voiceData.mapping?.[canonical(text)];
 ...
 return {speak,stop,mount,getState:()=>({voice,rate,lastName,spokenText,lastEngine,lastError,playing:!audio.paused||!!utterance,currentTime:audio.currentTime,recordings:voiceData.uniqueRecordings||0,synthetic:true,readingGuard:true,termSource:'catalog-v10',remoteVoicePossible:true,allAudioHumanReviewed:false})};
}
