...
export function initStability(ctx){
 ...
 async function capture(){
  ...
  g.fillStyle='rgba(248,252,245,.96)';g.fillRect(0,r.height-64,r.width,64);g.fillStyle='#345642';g.font='12px "Noto Sans CJK SC",sans-serif';g.fillText(mode+layer,14,r.height-41,r.width-28);g.font='10px sans-serif';g.fillText('V11 · Z-Anatomy / BodyParts3D · CC BY-SA 4.0 · 学习参考',14,r.height-20,r.width-28);g.restore();
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw Error('截图导出失败');
  const filename=`人体研习-${current.name}-${current.id}-V11-${new Date().toISOString().slice(0,10)}.png`.replace(/[\\/:*?"<>|]/g,'-');
