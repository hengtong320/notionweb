...
s=s.replace("const saved={section,scene,scroll:", "const saved={logical:layers.map(l=>{const a=systemsFor(l).map(k=>snapshots()[k]).filter(Boolean),on=a.filter(v=>v.on);return{id:l.id,on:on.length>0,opacity:on.length?on.reduce((n,v)=>n+(v.opacity??1),0)/on.length:(a[0]?.opacity??1)};}),display:$('sharedDisplay').value,section,scene,scroll:")
...
f.write_text(s)
f=p/'evidence-ui-v9.js';s=f.read_text();a="api.clearPointSelection?.();card.innerHTML=";assert a in s;s=s.replace(a,"api.clearPointSelection?.();window.dispatchEvent(new CustomEvent('atlas:selection',{detail:{kind:'meridian',id:ids.join(','),name:ids.map(id=>DATA.meridians[id]?.name||'头面补充').join('＋'),side:api.getState().tcmSide}}));card.innerHTML=");f.write_text(s)
...
s=s.replace("await p.locator('#saveLayerCombo').click();", "ck('Re-enabled layer is not accidentally invisible',await p.evaluate(()=>window.__ATLAS_TISSUES__.getState().systems.nervous.opacity>.05));await p.locator('#saveLayerCombo').click();")
