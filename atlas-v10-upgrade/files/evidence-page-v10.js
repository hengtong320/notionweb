import {evidenceData as DATA,POINT_TERMS,normalizeTerm} from './catalog-v10.js';
...
function show(id){
 selected=id;const p=DATA.points[id],m=DATA.meridians[id];
 if(tab==='point'&&p){$('detail').innerHTML=`<h2 id="focusHeading">${esc(p.name)}<small>${esc(p.pinyin)} · ${esc(id)}</small></h2><p><b>${esc(DATA.meridians[p.meridian]?.name||p.meridian)}</b> · ${esc(p.region)}</p><div class="actions"><a id="open3d" href="${esc(targetLink())}">在三维中查看相关部位 ↗</a></div>${evidenceBody(p)}`;}
 else if(tab==='meridian'&&m){$('detail').innerHTML=`<h2 id="focusHeading">${esc(m.name)}<small>${esc(m.pinyin)} · ${esc(id)}</small></h2><div class="actions"><a id="open3d" href="${esc(targetLink())}">在三维中查看这条经脉 ↗</a></div>${channelBody([id])}`;}
 else{$('detail').innerHTML='<h2>选择一个条目</h2><p>可搜索名称、正确拼音或编号，查看资料与三维观察范围。</p><a id="open3d" href="./">返回三维</a>';}
 record();for(const b of $('results').querySelectorAll('button'))b.classList.toggle('active',b.dataset.id===selected);
}
function render(){
 ...
 const list=Object.values(tab==='point'?DATA.points:DATA.meridians).filter(p=>(tab==='meridian'||!filter||p.meridian===filter)&&(!q||normalizeTerm([p.name,p.pinyin,p.id,p.code,...(POINT_TERMS[p.code]?.aliases||[])].join(' ')).includes(q)));
 $('count').textContent=`${list.length}条 · 名称完整，坐标审核状态单独记录`;$('results').replaceChildren();
...
$('channel').innerHTML='<option value="">全部经脉</option>'+Object.values(DATA.meridians).map(m=>`<option value="${esc(m.id)}">${esc(m.name)}</option>`).join('');
$('search').oninput=render;$('channel').onchange=render;
$('pointTab').onclick=()=>{tab='point';$('search').value='';render();show(DATA.points[selected]?selected:'PC6');};
