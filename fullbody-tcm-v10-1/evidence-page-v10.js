...
window.addEventListener('hashchange',()=>{const h=new URLSearchParams(location.hash.slice(1));tab=h.get('kind')==='meridian'?'meridian':'point';side=['left','right','midline','both'].includes(h.get('side'))?h.get('side'):'right';mode=h.get('mode')==='illustrative'?'illustrative':'strict';layer=['bones','muscles','nerves','compare'].includes(h.get('layer'))?h.get('layer'):'bones';render();show(h.get('id')||'PC6');});
