...
export function initStability(ctx){
 ...
 async function capture(){
  ...
  const mode=(current.kind==='point'||current.kind==='meridian'||learning.getState().enabled)?(learning.getState().precisionMode==='strict'?'资料查阅 · 未校准点线隐藏':'三维示意 · 穴位与经络位置未校准'):'解剖教学模型 · 非患者诊断';
  ...
  g.fillStyle='rgba(248,252,245,.96)';g.fillRect(0,r.height-64,r.width,64);g.fillStyle='#345642';g.font='12px "Noto Sans CJK SC",sans-serif';g.fillText(mode+layer,14,r.height-41,r.width-28);g.font='10px sans-serif';g.fillText('V10.1 · Z-Anatomy / BodyParts3D · CC BY-SA 4.0 · 学习参考',14,r.height-20,r.width-28);g.restore();
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw Error('截图导出失败');
  const filename=`人体研习-${current.name}-${current.id}-V10.1-${new Date().toISOString().slice(0,10)}.png`.replace(/[\\/:*?"<>|]/g,'-');
