...
export function initLearningEnhancements(ctx) {
 ...
 async function setSurfaceAttachment(on){
  ...
  try{
   for(const r of routeRecords){
    for(const p of r.data){if(!p.sourcePosition)p.sourcePosition=p.position.clone();p.position.copy(on?surfaceProjector.project(p.sourcePosition,4.1):p.sourcePosition);}
    for(const g of r.guides){
     if(!g.sourcePoints){g.sourcePoints=g.points.map(v=>v.clone());g.sourceGeometry=g.tube.geometry;}
     if(on&&!g.projected){g.projected=surfaceProjector.curve(g.sourcePoints);}
