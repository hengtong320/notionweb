...
const label=live?'live':engine===webkit?'webkit':'offline';
(async()=>{let browser,page;try{
 ...
 const state=()=>page.evaluate(()=>window.__FOOT_ATLAS__.getState()),settle=()=>page.waitForTimeout(1100);
 ...
 await page.locator('#labelsBtn').click();await settle();check('Foot labels show original 28 structures',await page.locator('.bone-label').count()===28);
 ...
 if(!s.labels)await page.locator('#labelsBtn').click();await settle();check('All 30 individual labels created',await page.locator('.bone-label').count()===30);await page.screenshot({path:path.join(out,label+'-04-exploded.png')});
